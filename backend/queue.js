...
(async () => {
  try {
    ...
    console.error(
      "❌ Failed to initialize JobScheduler:",
      error.message || error
    );
  }
})();
...
export async function addJobToQueue() {
  try {
    ...
    console.log(
      `🆕 Added Job: BullMQ ID = ${job.id}, DB ID = ${jobRecord.id}, Custom ID = ${jobRecord.customId}`
    );
...
export function initializeQueue(io) {
  ...
  const worker = new Worker(
    ...
    async (job) => {
      ...
      if (!dbJob) {
        console.error(
          `❌ Job ${job.id} NOT found in database! Skipping processing.`
        );
        return;
      }
  ...
  ); // By default it runs in FIFO config - First In - First Out. But for demo purpose I have configured it to run 4  jobs parallelly.

  // ✅ Handle Permanent Job Failure
  worker.on("failed", async (job, err) => {
    console.error(
      `❌ Job ${job.id} failed permanently after ${job.opts.attempts} attempts:`,
      err.message
    );
